...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'request') {
    ...
    // Manually compute next Bid since Bid is not AUTO_INCREMENT
    $nextBidStmt = $pdo->query("SELECT COALESCE(MAX(Bid), 0) + 1 AS nextBid FROM Bookings");
    $nextBidRow  = $nextBidStmt->fetch();
    $nextBid     = $nextBidRow['nextBid'];

    $insert = $pdo->prepare(
        "INSERT INTO Bookings (Bid, Date_Created, Start_Time, End_Time, Status, Iid, Cid, Rid)
         VALUES (:bid, :dateCreated, :start, :end, 'pending', :iid, :cid, :rid)"
    );
    $insert->execute([
        ':bid'         => $nextBid,
